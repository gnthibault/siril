...
__inline double xatan2(double y, double x) {
    ...
    r = mulsign(r, x);
    if (xisinf(x) || x == 0) r = RT_PI_2 - (xisinf(x) ? (sign(x) * (RT_PI_2)) : 0);
    if (xisinf(y)          ) r = RT_PI_2 - (xisinf(x) ? (sign(x) * (RT_PI*1/4)) : 0);
    if (             y == 0) r = (sign(x) == -1 ? RT_PI : 0);
...
__inline double xlog(double d) {
    ...
    if (xispinf(d)) x = RT_INFINITY;
    if (d < 0) x = RT_NAN;
    if (d == 0) x = -RT_INFINITY;
...
__inline double xpow(double x, double y) {
    ...
    result = xisnan(result) ? RT_INFINITY : result;
    result *=  (x >= 0 ? 1 : (!yisint ? RT_NAN : (yisodd ? -1 : 1)));

    double efx = mulsign(xfabs(x) - 1, y);
    if (xisinf(y)) result = efx < 0 ? 0.0 : (efx == 0 ? 1.0 : RT_INFINITY);
    if (xisinf(x) || x == 0) result = (yisodd ? sign(x) : 1) * ((x == 0 ? -y : y) < 0 ? 0 : RT_INFINITY);
    if (xisnan(x) || xisnan(y)) result = RT_NAN;
    if (y == 0 || x == 1) result = 1;
...
__inline double xsinh(double x) {
    ...
    y = xisinf(x) || xisnan(y) ? RT_INFINITY : y;
    y = mulsign(y, x);
    y = xisnan(x) ? RT_NAN : y;
...
__inline float xatan2f(float y, float x) {
    ...
    r = mulsignf(r, x);
    if (xisinff(x) || x == 0) r = RT_PI_F/2 - (xisinff(x) ? (signf(x) * (float)(RT_PI_F*.5f)) : 0);
    if (xisinff(y)          ) r = RT_PI_F/2 - (xisinff(x) ? (signf(x) * (float)(RT_PI_F*.25f)) : 0);
    if (              y == 0) r = (signf(x) == -1 ? RT_PI_F : 0);
...
__inline float xlogf(float d) {
    ...
    if (xispinff(d)) x = RT_INFINITY_F;
    if (d < 0) x = RT_NAN_F;
    if (d == 0) x = -RT_INFINITY_F;
