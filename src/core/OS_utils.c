...
#ifdef _WIN32
/**
 * This piece of code comes from glib.
 * It returns the second argument of comand line for windows.
 * @return
 */
char* win32_get_argv1(void) {
	 ...
	 * normal characters.
	 */
	while (cmdline[0] == 0x09 || cmdline[0] == 0x0a || cmdline[0] == 0x0c
			|| cmdline[0] == 0x0d || cmdline[0] == 0x20)
		cmdline++;
	...
	if (wargc > 1)
		utf8_buf = g_utf16_to_utf8(wargv[1], -1, NULL, NULL, NULL);
